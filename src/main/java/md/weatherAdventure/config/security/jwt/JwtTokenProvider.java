...
//старая дата которая просто дробавляет текующему году 1 час
        Date now = new Date();
// плюс 1 год и тот самый час
        Calendar cal = Calendar.getInstance();
        Date today = cal.getTime();
        cal.add(Calendar.YEAR, 1); // to get previous year add -1
        Date nextYear = cal.getTime();

        Date validity = new Date(nextYear.getTime() + validityInMilliseconds);
    ...
    public void setId(JwtUser  user) {
        this.id = user.getId();
    }
}
